...
pub fn turn_handler(key: Res<Option<VirtualKeyCode>>,
    ...
    let newStage = match (updatedStage, *key) {
        (TurnStage::Swinging(swing), Some(VirtualKeyCode::Space)) => {
            match swing {
                Swing::Start(deg) => {
                    Some(TurnStage::Swinging(Swing::Power(deg, 0.)))
                },
                Swing::Power(deg, pow) => {
                    Some(TurnStage::Swinging(Swing::Accuracy(deg, pow, 1.0)))
                },
